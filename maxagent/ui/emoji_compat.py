...
EMOJI_FALLBACK_TABLE = {
    ...
    '👁': '◉',       # 显示 / 查看（同心圆作"眼睛"替身）
    '👁️': '◉',
    '🙈': '⊝',       # 隐藏 / 不可见（带斜线圆，"封起来"的暗示）—— 与 👁 配对
    '🔌': '⚡',       # 测试连接（闪电做"电气连接"暗示）
    '💾': '▣',       # 保存 / 应用（带框方块，保存图标的极简版）
    '📂': '▤',       # 打开文件夹（横线纹方块，"目录列表"暗示）
    '📁': '▤',
    # —— 资源管理（我的资源 / 工具 / 技能 / 导入导出）—— #
    # 这一组都是 SMP 平面 emoji，PySide2 + Win 字体回退族中往往没有
    # 对应字形，必须给 BMP 兜底，否则按钮上会出现"豆腐块"。
    '📦': '◫',       # 资源包 / 我的资源（带格方块，"打包"暗示）
    '📤': '↥',       # 导出 / 上传（向上箭头）
    '📥': '↧',       # 导入 / 下载（向下箭头）
    '🎓': '◇',       # 技能 / 学习（菱形，"勋章/认证"的极简感）
    '🧰': '✱',       # 工具箱（与"工具/齿轮"统一花型符）
    '📎': '⌬',       # 添加附件 / 别针（六边形带轴，"夹子"的视觉暗示）
    '📷': '◉',       # 相机 / 选择图片（同心圆复用，与 👁 区分由文案承载）
    '✂': '✂',       # 剪刀 / 截图（U+2702 在 BMP 但常被 VS16 emoji 化，
    '✂️': '✂',      # 显式映射成裸 BMP 形保证不被 emoji 字体接管）
    '🗑️': '✕',      # 删除（与 🗑 统一指向 ✕）
}
