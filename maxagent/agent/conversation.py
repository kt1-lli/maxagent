...
def build_default_system_prompt(employee_name=None, force_dcc=None):
    ...
    body = (
        ...
        '\n'
        '【🔧 工具调用通用约定】\n'
        '27. 所有工具参数必须符合 JSON Schema；当参数描述要求'
        '"[x,y,z]" 形式的数组时，优先使用 JSON 字符串传递（如 '
        '"[50,60,70]"），除非 schema 显式声明为 array 类型。\n'
        '28. 调用 create_* 工具后若涉及位置/旋转，必须继续调用 '
        'move_object / rotate_object 或 run_python 完成摆放，不能把'
        '对象留在世界原点。\n'
        '29. 工具返回失败时，先读取 error 字段，对照该工具的'
