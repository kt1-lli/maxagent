...
def _parse_scene_from_jwt(jwt_token: str) -> str:
    """从 JWT payload 中提取 scene 字段（内部辅助函数）"""
...
def parse_current_agent_id(jwt_token: str, headers: dict) -> str:
    """解析当前对话的真实 agent_id。
    先从 JWT 中提取 scene，再通过 agents/list 接口查询对应的 agent_id。
    """
    scene = _parse_scene_from_jwt(jwt_token)
    ...
    if not agents:
        print(f"错误：未找到 scene={scene} 对应的 Agent，请确认当前对话的智能体配置。", file=sys.stderr)
        sys.exit(1)
    agent_id = agents[0].get("id")
    if not agent_id:
        print(f"错误：agents/list 返回的数据中缺少 id 字段（scene={scene}）", file=sys.stderr)
        sys.exit(1)
    return agent_id
