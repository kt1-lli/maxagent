...
- **🔴 高危操作**：执行后将立即覆盖当前未发布的草稿，所有未发布修改永久丢失，无法恢复
- **⚠️ AI 必须在执行前向用户明确说明上述风险，并获得用户的明确确认，才能调用此脚本；若用户未明确表示确认，不得执行**
...
- **🔴 高危操作**：发布后所有用户的正式对话立即使用新版本，无法撤销
- 脚本会自动检查草稿是否 `is_stale`；若 `is_stale=true`，须额外告知用户"草稿已过期，发布将覆盖最新版本改动"
- **⚠️ AI 必须在执行前向用户明确说明上述风险，并获得用户的明确确认，才能调用此脚本；若用户未明确表示确认，不得执行**
