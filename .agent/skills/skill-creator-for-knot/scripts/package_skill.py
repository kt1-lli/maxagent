...
示例：
    ...
    python utils/package_skill.py skills/public/my-skill /path/to/output

默认输出目录：当前工作目录（工作区根目录）
"""
...
def package_skill(skill_path, output_dir=None):
    ...
    if output_dir:
        ...
        output_path = DEFAULT_OUTPUT_DIR.resolve()
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python utils/package_skill.py skills/public/my-skill /path/to/output")
        sys.exit(1)
    ...
    if output_dir:
        ...
        print(f"   输出目录：{DEFAULT_OUTPUT_DIR.resolve()}（默认）")
    print()
